...
showMobileInfo.addEventListener("click", function (e) {
  document.querySelector(".infoContentR")
    ? document.querySelector(".infoContentR").remove()
    : true;
  ...
  if (e.target.nodeName == "SPAN") {
    ...
    for (let i = 0, c = e.currentTarget.children; i < c.length; i++) {
      ...
      if (e.target == c[i]) {
        ...
        document.querySelector(".map-overlay h6").innerHTML =
          " Local Law " +
          c[i].attributes["data-dispname"].value +
          " " +
          document.querySelector("body span.msg" + vLLAttrib).innerText;
        ...
        for (let x = 0; x < _vtemp.length; x++) {
          _vtemp[x].innerHTML = document
            .querySelector(".selectedLL")
            .getAttribute("data-dispname");
        }
      } else {
    ...
    if (locallaws[0]) {
      for (let x = locallaws.length - 1; x > -1; x--) {
        ...
        document.querySelector(
          ".ll" + locallaws[x].name + " .lead em"
        ).innerHTML = locallaws[x].desc;
        if (locallaws[x].who.length > 0) {
          for (let ctr = 0, t = locallaws[x].who.length; ctr < t; ctr++) {
            let el = document.createElement("li");
            let elcontent = document.createTextNode(
              locallaws[x].who[ctr].description
            );
            el.appendChild(elcontent);
            document
              .querySelector(".ll" + locallaws[x].name + " #who ol")
              .appendChild(el);
          }
        } else {
          document.querySelector(
            ".ll" + locallaws[x].name + " #who"
          ).innerHTML =
            document.querySelector(".ll" + locallaws[x].name + " #who")
              .innerHTML + " tbd";
        }
        if (locallaws[x].exceptions.length > 0) {
          for (
            let ctr = 0, t = locallaws[x].exceptions.length;
            ctr < t;
            ctr++
          ) {
            let el = document.createElement("li");
            let elcontent = document.createTextNode(
              locallaws[x].exceptions[ctr].description
            );
            el.appendChild(elcontent);
            document
              .querySelector(".ll" + locallaws[x].name + " #exceptions ol")
              .appendChild(el);
          }
        } else {
          document.querySelector(
            ".ll" + locallaws[x].name + " #exceptions"
          ).innerHTML =
            document.querySelector(".ll" + locallaws[x].name + " #exceptions")
              .innerHTML + " tbd";
        }
        if (
          document.querySelector(".selectedLL").getAttribute("data-ll") ==
          locallaws[x].name
        ) {
          document.querySelector(".ll" + locallaws[x].name).style.display =
            "block";
        }
      }
    ...
    map.on("load", function () {
      ...
      map.on("click", "97", function (e) {
        if (e.features.length > 0) {
          if (clickStateId) {
            map.setFeatureState(
              { source: "_97Source", id: clickStateId },
              { click: false }
            );
          }
      ...
      map.on("click", "33", function (e) {
        if (e.features.length > 0) {
          if (clickStateId) {
            map.setFeatureState(
              { source: "_33Source", id: clickStateId },
              { click: false }
            );
          }
          clickStateId = e.features[0].id;
...
map.on("click", function (e) {
  ...
  if (lots_geojson.length > 0) {
    document.getElementById("lotdetails").innerHTML =
      "<p>" +
      "BBL: " +
      lots_geojson[0].properties.BBL_MapPLU +
      "</p>" +
      "<p>" +
      "Borough: " +
      lots_geojson[0].properties.BoroughNam +
      "</p>" +
      "<p>" +
      "Block: " +
      lots_geojson[0].properties.Block_1 +
      "<p>" +
      "Lot: " +
      lots_geojson[0].properties.Lot_1 +
      "</p>" +
      "<p>" +
      "Address: " +
      lots_geojson[0].properties.Street_Num +
      " " +
      lots_geojson[0].properties.Street_Nam +
      "</p>" +
      "<p>" +
      "Number of Buildings: " +
      lots_geojson[0].properties.Building_1 +
      "</p>" +
      "<p>" +
      "Gross Square Footage: " +
      lots_geojson[0].properties.DOF_Gross_ +
      "</p>" +
      "<p>" +
      "Building Class: " +
      lots_geojson[0].properties.Building_C +
      "</p>" +
      "<p>" +
      "Tax Class: " +
      lots_geojson[0].properties.Tax_Class +
      "</p>" +
      //((lots_geojson[0].properties.DCAS_City) ? '<p>'+"City-owned: "+ ( (lots_geojson[0].properties.DCAS_City=="Y") ? "Yes":"No") + "</p>" : "") + //removed   @ Tim's req

      (lots_geojson[0].properties.dobnyc_G_1
        ? "<p>" +
